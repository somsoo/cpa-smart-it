...
def create_text_thumbnail(text, filename_prefix):
    try:
        ...
        if not lines: lines = ["폰가비 S급 중고폰 디바이스 최저가 구매", "핵심 실무 가이드"]
...
def generate_post(campaign, keyword):
    ...
    draft_prompt = f"""당신은 스마트 실버케어 전문 복지상담사 및 통신 컨설턴트입니다.
...
{{
  ...
  "thumb_hook": "{keyword}\\n핵심 실무 가이드 (줄바꿈 \\n)",
  "vibe_keywords": "elderly technology phone",
...
---METADATA_END---"""
    ...
    vibe_keywords = meta.get('vibe_keywords', 'elderly technology phone')
